...
class Ships:
    def __init__(self):
        ...
        self.ships = getShips('Git/ShipCatalog/sources/allships.txt')
        ...
        #self.byProductionStatus = OrderedDict(sorted(self.ships.items(), key=lambda item: (['flight-ready', 'hangar-ready', 'in-production', 'in-concept', 'announced'].index(item[1]['production status']), item[1]['model'])))
